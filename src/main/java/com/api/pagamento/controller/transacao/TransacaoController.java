...
public class TransacaoController {
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = SucessConstants.SUCESSO_200_OPERACAO_REALIZADA, response = TransacaoResponseTO.class),
	...
	public ResponseEntity<Object> buscarTransacao(@PathVariable Long id) {
		...
		try {
			TransacaoResponseTO transacaoDTO = transacaoDtoService.buscarTransacao(id);
	...
	public ResponseEntity<Object> listarTransacoes() {
		...
		try {
			List<TransacaoResponseTO> transacaoDTOS = transacaoDtoService.listarTransacoes();
	...
	public ResponseEntity<Object> pagar(@RequestBody @Valid TransacaoRequestDTO request) {
		...
		try {
			TransacaoResponseTO transacaoDTO = transacaoDtoService.pagar(request);
			return ResponseEntity.ok().body(transacaoDTO);
		} catch (NotFoundException | BadRequestException ex) {
			throw ex;
		} catch (Exception ex) {
	...
	public ResponseEntity<Object> estornar(@PathVariable Long id) {
		...
		try {
			TransacaoResponseTO transacaoDto = transacaoDtoService.estornar(id);
			return ResponseEntity.ok().body(transacaoDto);
		} catch (NotFoundException | BadRequestException ex) {
			throw ex;
		} catch (Exception ex) {
